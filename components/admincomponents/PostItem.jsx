...
const PostItem = ({ post }) => {
  ...
  return (
    <div className="w-[90%] bg-white text-[#52796F] py-6 px-4 align-middle m-auto mb-2 rounded-md shadow-[20px_20px_60px_#878787] hover:scale-105 transition ease-out delay-75">
      ...
      <div className="flex justify-between">
        ...
        <span className="flex justify-center items-center gap-2">
          <span className="flex gap-1 text-xl">
            <span className="">{post.heartCount || 0}</span>
          ...
          <span className="flex">
            ...
            <span className="text-xl">{post?.commentCount}</span>
          </span>
        </span>
      </div>
      ...
      {admin && (
        <div className="flex gap-2 mt-2">
          <Link href={`/admin/${post.slug}`}>
            <button className="relative inline-flex items-center justify-center px-4 py-2 overflow-hidden font-mono font-medium tracking-tighter text-white bg-[#354F52] rounded-lg group">
          ...
          {post.published ? (
            <p className="text-green-500 mt-2">Live</p>
          ) : (
            <p className="text-red-600 mt-2">Unpublished</p>
          )}
        </div>
      )}
    </div>
